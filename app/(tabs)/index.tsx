            ...
            <View>
                ...
                <View style={styles.heroContent}>
                  <View>
                    ...
                    <Text style={styles.heroSub}>
                      Hey {firstName} · {totalGifts} {totalGifts === 1 ? 'gift' : 'gifts'} collected
                    </Text>
                  </View>
                  <Pressable
              ...
              {showFilters && (
                <View style={[styles.filterPanel, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                  <Text style={[styles.filterSectionLabel, { color: colors.muted }]}>Occasion</Text>
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 14 }}>
                    <Pressable
                      onPress={() => updateFilter('occasion', 'all')}
                      style={[styles.filterChip, {
                        backgroundColor: filters.occasion === 'all' ? colors.primary : colors.surface2,
                        borderColor: filters.occasion === 'all' ? colors.primary : colors.border,
                      }]}
                    >
                      <Text style={[styles.filterChipText, { color: filters.occasion === 'all' ? '#fff' : colors.muted }]}>All</Text>
                    </Pressable>
                    {OCCASIONS.map((occ) => (
                      <Pressable
                        key={occ.value}
                        ...
                        style={[styles.filterChip, {
                          backgroundColor: filters.occasion === occ.value ? colors.primary : colors.surface2,
                          borderColor: filters.occasion === occ.value ? colors.primary : colors.border,
                        }]}
                      >
                        <Text style={styles.filterChipEmoji}>{occ.emoji}</Text>
                        <Text style={[styles.filterChipText, { color: filters.occasion === occ.value ? '#fff' : colors.muted }]}>
                          {occ.label}
                        </Text>
                      </Pressable>
                    ))}
                  </ScrollView>
                  <Text style={[styles.filterSectionLabel, { color: colors.muted }]}>Sort by</Text>
              ...
              {filteredGifts.length > 0 && (
                <View style={styles.sectionHeader}>
                  <Text style={[styles.sectionLabel, { color: colors.muted }]}>
                    {filters.search || filters.occasion !== 'all' ? `${filteredGifts.length} results` : 'All Gifts'}
                  </Text>
                </View>
              )}
            </View>
