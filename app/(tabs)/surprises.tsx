...
function StatusBadge({ deliveryDate, isUnlocked }: { deliveryDate: string; isUnlocked: boolean }) {
...
function SurpriseCard({ item, isSentByMe, onEdit, onDelete }: {
  ...
  const iconColor = item.isUnlocked ? '#22C55E' : available ? '#8B5CF6' : colors.muted;
  const iconName: any = item.isUnlocked ? 'gift.fill' : available ? 'sparkles' : 'lock.fill';
  ...
  return (
    ...
    >
      ...
      {(available || item.isUnlocked) && (
        <LinearGradient
          colors={item.isUnlocked ? ['#22C55E', '#4ADE80'] : ['#8B5CF6', '#EC4899']}
          start={{ x: 0, y: 0 }}
          end={{ x: 1, y: 0 }}
      ...
      <View style={styles.cardBody}>
        ...
        <View style={styles.cardInfo}>
          ...
          <StatusBadge deliveryDate={item.deliveryDate} isUnlocked={item.isUnlocked} />
        </View>

        {/* Right icon */}
        <View style={[styles.iconWrap, {
          backgroundColor: item.isUnlocked ? '#22C55E18' : available ? '#8B5CF618' : colors.surface2,
          borderColor: item.isUnlocked ? '#22C55E33' : available ? '#8B5CF633' : colors.border,
        }]}>
        ...
        <View style={[styles.cardActions, { borderTopColor: colors.border }]}>
          {onEdit && (
            <Pressable
              ...
              style={[styles.actionChip, { borderColor: colors.primary + '44', backgroundColor: colors.primary + '11' }]}
            >
          ...
          {onDelete && (
            <Pressable
              ...
              style={[styles.actionChip, { borderColor: '#EF444444', backgroundColor: '#EF444411' }]}
            >
...
function SectionEmptyCard({ emoji, text, actionLabel, onAction }: {
  emoji: string;
  text: string;
  actionLabel?: string;
  onAction?: () => void;
  ...
  return (
    <View style={[styles.sectionEmpty, { backgroundColor: colors.surface, borderColor: colors.border }]}>
      ...
      <Text style={[styles.sectionEmptyText, { color: colors.muted }]}>{text}</Text>
      {actionLabel && onAction && (
        <Pressable onPress={onAction} style={{ marginTop: 8 }}>
          <Text style={[styles.sectionEmptyAction, { color: colors.primary }]}>{actionLabel}</Text>
        </Pressable>
      )}
    </View>
  );
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        >
          ...
          <View style={styles.heroTop}>
            ...
            <Pressable
              onPress={handleSend}
              style={({ pressed }) => [styles.sendFab, pressed && { transform: [{ scale: 0.9 }] }]}
            >
              <LinearGradient
        ...
        <View style={styles.section}>
          ...
          {incoming.length === 0 ? (
            <SectionEmptyCard
              emoji="📭"
              text="No incoming surprises yet"
            />
          ) : (
            incoming.map((item) => (
        ...
        {/* Sent section */}
        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: colors.muted }]}>Sent</Text>
          {sent.length === 0 ? (
            <SectionEmptyCard
              emoji="📤"
            ...
            sent.map((item) => (
              <SurpriseCard
                ...
                item={item}
                isSentByMe={true}
                onEdit={item.isUnlocked ? undefined : () =>
                  router.push({ pathname: '/edit-surprise/[id]' as any, params: { id: item.id } })
                }
...
const styles = StyleSheet.create({
  ...
  labelText: { fontSize: 10, fontWeight: '700', letterSpacing: 0.5, textTransform: 'uppercase' },
