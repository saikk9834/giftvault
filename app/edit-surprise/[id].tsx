...
export default function EditSurpriseScreen() {
  ...
  useEffect(() => {
    if (surprise) {
      ...
      const d = surprise.deliveryDate instanceof Date
  ...
  const handleSave = async () => {
    ...
    if (!puzzle.trim() || !answer.trim()) {
      if (Platform.OS === 'web') {
        ...
        Alert.alert('Add Puzzle', 'Please fill in both the riddle and the answer.');
      }
      return;
    }
  ...
  return (
    ...
    >
      ...
      <View style={[styles.header, { paddingTop: insets.top + 8, borderBottomColor: colors.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.foreground }]}>Edit Surprise</Text>
        <View style={{ width: 22 }} />
      </View>
      ...
      {isLoading ? (
        ...
        >
          <Text style={[styles.recipientLabel, { color: colors.muted }]}>
            To: <Text style={{ color: colors.foreground, fontWeight: '700' }}>{surprise.recipientName}</Text>
          </Text>

          <Text style={[styles.stepLabel, { color: colors.muted }]}>Gift Description</Text>
          <TextInput
            value={giftContent}
          ...
          <Text style={[styles.stepLabel, { color: colors.muted }]}>Puzzle / Riddle</Text>
          <TextInput
            value={puzzle}
          ...
          {puzzleImageUri ? (
            <View style={styles.imageWrapper}>
              <Image source={{ uri: puzzleImageUri }} style={styles.imagePreview} contentFit="cover" />
              <View style={styles.imageActions}>
                <Pressable
                  onPress={pickPuzzleImage}
                  style={[styles.imageActionBtn, { borderColor: colors.primary + '55', backgroundColor: colors.primary + '11' }]}
                >
                ...
                <Pressable
                  ...
                  style={[styles.imageActionBtn, { borderColor: (colors.error ?? '#EF4444') + '55', backgroundColor: (colors.error ?? '#EF4444') + '11' }]}
                >
                  <IconSymbol name="trash" size={13} color={colors.error ?? '#EF4444'} />
                  <Text style={[styles.imageActionText, { color: colors.error ?? '#EF4444' }]}>Remove</Text>
                </Pressable>
              </View>
            </View>
          ) : (
          ...
          <Text style={[styles.stepLabel, { color: colors.muted }]}>Delivery Date</Text>
          <TextInput
            value={deliveryDate}
